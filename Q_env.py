...
class Env(tk.Tk):
    def __init__(self):
        ...
        self.epoch = 0
        self.geometry('{0}x{1}'.format(WIDTH * UNIT + 60, HEIGHT * UNIT + 140))
        self.shapes = self.load_images()
        self.canvas,  self.label = self._build_canvas()
        self.texts = []
    ...
    def _build_canvas(self):
        ...
        label = tk.Label(self, text='할인율 : 0.9  학습률 : 0.01  입실론 : 0.1 이 적용된 모델의 ' + str(self.epoch) + ' 세대 입니다.', font=('',15), foreground = 'red')
        label.pack(pady=7)


        self.label.place(x=0, y=40,width = WIDTH * UNIT + 60, height = HEIGHT * UNIT + 100)
        # 캔버스 가운데 정렬
    ...
    def label_update(self) :
        self.epoch += 1
        self.label.config(text='할인율 : 0.9  학습률 : 0.01  입실론 : 0.1 이 적용된 모델의 ' + str(self.epoch) + ' 세대 입니다.', font=('',15))
    
    def load_images(self):
        background = PhotoImage(Image.open("../img/guard.png").resize((WIDTH * UNIT + 60, HEIGHT * UNIT + 100)))
            ...
            for y in range(HEIGHT):
                for action in range(0, 4):
                    ...
                    if str(state) in q_table.keys():
                        ...
                        self.text_value(y, x, round(temp, 5), action)
